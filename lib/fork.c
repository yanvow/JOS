...
pgfault(struct UTrapframe *utf)
{
  ...
  if (sys_page_alloc(0, (void *)PFTEMP, perm) < 0) {
    panic("In pgfault, sys_page_alloc error");
  }
  memcpy((void *) PFTEMP, addr, PGSIZE);
  if (sys_page_map(0, (void *) PFTEMP, 0, addr, perm) < 0) {
    panic("In pgfault, sys_page_map error");
  }
...
fork(void)
{
  ...
  if ((r = sys_env_set_pgfault_upcall(eid, _pgfault_upcall)) < 0)
    panic("In fork, sys_env_set_pgfault_upcall error: %e", r);

  if ((r = sys_page_alloc(eid, (void *)(UXSTACKTOP - PGSIZE), PTE_W | PTE_U)) < 0)
    panic("In fork, sys_page_alloc error: %e", r);

  if ((r = sys_env_set_status(eid, ENV_RUNNABLE)) < 0)
    panic("In fork, sys_env_set_status error: %e", r);
